...
in each instrument at time $τ_j$. Trades accumulate to a _position_, $Δ_t =
\sum_{τ_j < t}Γ_j = \sum_{s < t}Γ_s$ where $Γ_s
= Γ_j$ when $s = τ_j$.  Note a trade at time $t$ is not
...
The one-period model assumes $T = \{t_0, t_1\}$. Let $x = X_{t_0}$ and $X = X_{t_1}$.
The no arbitrage condition is there does not exist $γ\in\RR^I$
with $-γ\cdot x > 0$ and $γ\cdot X\ge 0$.
In this case the one-period Fundamental Theorem of Asset Pricing states that there is no
