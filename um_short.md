...
$\Gamma_j:\AA_{τ_j}\rightarrow \RR^{I}$ indicating the number of shares to trade
in each instrument at time $τ_j$. Trades accumulate to a _position_
$\Delta_t = \sum_{τ_j < t}\Gamma_j = \sum_{s < t}\Gamma_s$ where $\Gamma_s = \Gamma_j$ when $s = τ_j$. 
...
$t > τ_0$, and $\sum_j \Gamma_j = 0$; you make money on the first trade and
...
$X_t D_t = E_t[(X_u + C_u) D_u]$ and $\Delta_t + \Gamma_t = \Delta_u$ 
$$
\begin{aligned}
...
\end{aligned}
$$
where we use $\Delta_u\cdot C_u =  \Gamma_u\cdot X_u + A_u$.
...
Since $V_{τ_0} = \Gamma_{τ_0} \cdot X_{τ_0}$, $A_{τ_0} = - \Gamma_{τ_0} \cdot X_{τ_0}$
...
a hedge, $(\bar{\Gamma}_t)_{t\in T}$, that replicates these amounts then the value
of the derivative is the cost of setting up the initial hedge: $\bar{\Gamma}_0\cdot X_0$.
...
cost of the initial hedge since $V_0 = \bar{\Gamma}_0\cdot X_0$.
...
Since $\Gamma_0\cdot X_0 = V_0$ the initial hedge is $\Gamma_0 = dV_0/dX_0$.

The hedge at time $t$ is similarly determined.
Since $(\Delta_t + \Gamma_t)\cdot X_t = V_t$ we have $\Delta_t + \Gamma_t =
dV_t/dX_t$, where the last term is the Fr&#233;chet derivative.  Since 
the position $\Delta_t$ is known at time $t$ this determines the trades:
