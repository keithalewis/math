...
quantities can be defined in terms of prices.
...
It costs $\xi^Tx$ to acquire the portfolio at the beginning of the period
and returns $\xi^T X(\omega)$ when sold at the end of the period if $\omega\in\Omega$ occurs.

The _realized return_ is $R(\xi) = \xi\cdot X/\xi\cdot x$.
