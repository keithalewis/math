...
A subset of a vector space closed under positive scalar multiplication and
vector addition is a _cone_. The previous two exercises show the set
of arbitrage portfolios is a cone.
...
${\cone X(\Omega) = \{\sum_j X(\omega_j) D_j\mid D_j > 0\}}$.
If ${\xi\cdot X\ge0}$ on $\Omega$ and $x$ is in the cone
then $\xi\cdot x = \sum_j \xi\cdot X(\omega_j) D_j\ge0$ so there is no arbitrage.

__Exercise__. _Show this holds for every point in the closure of the cone_.

_Hint_: If $x_n\to x$ and $\xi\cdot x_n\ge0$ then $\xi\cdot x\ge0$.
...
= -\xi\cdot x + \langle \xi\cdot X, D\rangle}$
...
_Hint_: $PQ = 0$ if and only if $P = Q$.

This shows if $P = \sum_i x_i P_i$ then $\Pi_j(P) = x_j \Pi$.
We write $x_j = \Pi_j(P)/\Pi$ so
$$
...
$$
x = \frac{xX(L)X(K)(H)}{\Pi} O + \frac{OxX(K)X(H)}{\Pi} X(L)
+ \frac{OX(L)xX(H)}{\Pi} X(K) + \frac{OX(L)X(K)x}{\Pi} X(H)
$$
...
$$
\begin{aligned}
OX(L)xX(K) &= O(R\,\rho + L\,\sigma)(\rho + s\,\sigma + v\,\kappa)X(K) \\
	&= (Rs\,O\rho\sigma + Rv\,O\rho\kappa + L\,O\sigma\rho + Lv\,O\sigma\kappa)X(K) \\
...
$$
\begin{aligned}
OX(L)X(H)x &= O(R\,\rho + L\,\sigma)(R\,\rho + H\,\sigma + (H - K)\,\kappa)x \\
	&= RH\,O\rho\sigma + R(H - K)\,O\rho\kappa
		+ LR\,O\sigma\rho + L(H - K)\,O\sigma\kappa \\
	&= ((RH - LR)\,O\rho\sigma + R(H - K)\,O\rho\kappa +  L(H - K)\,O\sigma\kappa)
		(\rho + s\sigma + v\kappa) \\
