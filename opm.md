...
describing a financial market over a single time period.  It specifies the
prices of available instruments at the beginning of the period and
their cash flows at the end depending on what occurred.
Prices are geometrically constrained by cash flows
...
of either 1 or 3. The model is $x = (1, 1)$
and ${C(\omega) = (2, \omega)}$ where ${\omega\in\{1,3\} = \Omega}$.
...
__Exercise__. _Show $\sum_{j=0}^k \frac{P_0\cdots R \cdots P_k} = P_0\cdots P_k$_.
