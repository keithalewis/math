...
__Exercise__. _If $X$ is standard normal show ${E[\max\{k - F, 0\}] = kN(x) - fN(x - s)}$
where $N$ is the standard normal cumulative distribution and ${x = (\log(k/f) + s^2/2)/s}$_.

_Hint_: Use $E[\exp(N)] = \exp(E[N] + \Var(N)/2)$ if $N$ is normal and
$E[\exp(N) g(M)] = E[\exp(N)] E[g(M + \Cov(M,N))]$ if $N$ and $M$
are jointly normal.

__Exercise__. _Show if $F > 0$ that $E[{\max\{k - F, 0\}] = kP(X\le x) - fP^*(X\le x)}$
where ${x = (\log(k/f) + \kappa(s))/s}$
...
\begin{aligned}
    ...
    &= -\partial_f P^*(X \le x) \\
    &= -\partial_x P^*(X \le x)\partial_f x \\
    &= -\partial_x P^*(X \le x)/\partial_x f \\
    &= \partial_x P^*(X \le x)/f s\\
\end{aligned}
$$
