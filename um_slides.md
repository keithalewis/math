...
* Amount (_A_): shares, notional, contracts, physical quantity, units,...
...
* _Price_ is $X = a/a'$ (after the trade).
...
Prices depend on time, the instruments, the entities,
and the amount the buyer wants to acquire, among other things.

* The trade $(t;i,a'X,e;i',a',e')$ is available to $e$ from $e'$,
...
A _market model_ specifies possible future prices and cash flows of instruments.
...
A _trading strategy_ specifies when, what, and how much to trade.
...
$$
Since $V_0 = Γ_0\cdot X_0 = -A_0$, $A_0\le 0$ so
...
A _zero coupon bond_ $D(u)$ has $C_u = 1$.

* $X_t D_t = E_t[D_u]$ so $X_t D_t = D_t(u)D_t = E_t[D_u]$,
$$
...
A _forward rate agreement_ $F^δ(t,u)$ has $C_t = -1$, $C_u = 1 + fδ$,
where $δ$ is the dcf for the interval from $t$ to $u$.
...
$$
so $X_t = M_t/D_t = (e^{ρt}, se^{σ B_t + (ρ - σ^2)t/2})$.
...
Given a set of amounts $A_j$ at $t_j$ how do we find a trading strategy that
produces these?

* $V_0 = E[\sum_j A_j D_{t_j}] = Γ_0\cdot X_0$ so $D_{X_0}V_0= Γ_0$.
* $V_j = E_j[\sum_{k > j} A_k D_{t_k}] = (Δ_j + Γ_j)\cdot X_j$ so
$D_{X_j}V_j = Δ_j + Γ_j$ (Frechet derivative.)
...
Monte Carlo all the things! We have Moore's Law on our side and can put
the ML headcount to good use.
