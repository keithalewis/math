...
More generally, let the bond have realized return $R$, the stock has initial price $s$ and
final price $\omega$ where $\omega\in\{S^-,S^+\}$. Assume an European option has price $c$
and pays $\nu(\omega)$ at the end of the period.
The one-period
model is $x = (1, s, c)$ and $X(\omega) = (R, \omega, \nu(\omega))$.
The range of $X$ is  $X(\Omega) = \{X(S^-), X(S^+)\}$  and
the smallest cone containing the range of $X$ consists of
$\{aX(S^-) + bX(S^+)\mid a,b\ge 0\}$.
If $x = aX(S^-) + bX(S^+)$ then $1 = aR + bR$ and $s = aS^- + bS^+$ so
$a = (S^+ - R)/R(S^+ - S^-)$ and $b = (R - S^-)/R(S^+ - S^-)$, hence
$c =  \nu(S^-)(S^+ - R)/R(S^+ - S^-) + \nu(S^+)(R - S^-)/R(S^+ - S^-)$.

If we let $\Omega = [S^-,S^+]$ be an interval we get the same answer.

__Exercise__. _Show $X(\omega)$ is a linear combination of $X(S^-)$ and
$X(S^+)$ with positive coefficients if $\omega\in[S^-, S^+]$_.

This shows the smallest cone generated by $X([S^-,S^+])$ equals the
smallest cone generated by $X(S^-)$ and $X(S^+)$.
